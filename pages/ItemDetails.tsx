  ...
  return (
    <div className="relative animate-in fade-in slide-in-from-right-4 duration-500 bg-slate-50 dark:bg-black min-h-screen">
      ...
      <div className="px-6 -mt-32 relative z-10 pb-20 max-w-4xl mx-auto">
        ...
        {/* Reading progress + mark-finished / reset buttons. Layout intentionally
        ...
        {(progressPct > 0 || item.formats.some(f => !f.external && f.allowReading !== false)) && (
          <div className="bg-white dark:bg-[#1c1c1e] p-5 rounded-[2.5rem] border border-slate-100 dark:border-white/10 shadow-sm mt-4 px-6">
            ...
            <div className="flex items-center gap-2 mb-3">
              <p className="text-[10px] uppercase font-black text-slate-400 dark:text-slate-500 tracking-widest">{t.progress}</p>
            ...
            {/* Action row — appears beneath the bar so it can't crowd the label.
                Each button takes equal width; when only one is visible it fills
                the row by itself. */}
              ...
              <div className="flex gap-2 mt-4">
                {progressPct < 95 && (
                  <button
                    ...
                    className="flex-1 py-2.5 bg-green-50 dark:bg-green-500/15 text-green-700 dark:text-green-300 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-green-100 dark:hover:bg-green-500/25 active:scale-95 transition-all flex items-center justify-center gap-1.5"
                  >
                    <CheckCircle2 size={13} strokeWidth={3} /> {t.markFinished}
                  </button>
                )}
                {progressPct > 0 && (
                  <button
                    onClick={handleResetProgress}
                    className="flex-1 py-2.5 bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-white/15 active:scale-95 transition-all flex items-center justify-center gap-1.5"
                  >
                    <RotateCcw size={13} strokeWidth={3} /> {t.resetProgress}
                  </button>
                )}
              </div>
            )}
